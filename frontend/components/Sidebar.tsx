...
import { GoalIcon, TickupLogoIcon } from './Icons';
...
export const Sidebar: React.FC<SidebarProps> = (props) => {
  const { currentUser, onLogout, onEditProfile, onViewProfile } = props;
  ...
  return (
    <aside className={`fixed top-0 right-0 h-full z-30 flex-col transition-all duration-300 ease-in-out hidden md:flex w-64 ${styles.aside}`}>
      <div className={`px-3 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between`}>
          ...
          <span className="font-bold text-lg">TickUp OKR</span>
      </div>
          ...
          <ul className="space-y-1">
            <li>
                ...
                >
                    ...
                    <span className="mr-3 whitespace-nowrap transition-opacity duration-300">اهداف</span>
                </button>
            </li>
          </ul>
      </nav>
